    ...
    bool CheckCollision(Player* pPlayer)
    {
        ...
        for (int i = 0; i < BAR_COUNT; ++i)
        {
            if (pPlayer->GetDy() > 0        // 점프가 가능한 상황
                ...
                && pPlayer->GetX() < vBar[i].x + imgWidth   // 개구리의 x좌표가 발판의 가로 길이보다 작은
                && pPlayer->GetY() + pPlayer->GetHeight() > vBar[i].y   // 개구리의 y좌표에 현재 높이를 더한게 발판의 y좌표보다 높을 때
                && pPlayer->GetY() + pPlayer->GetHeight() < vBar[i].y + 10) // 개구리의 y좌표에 현재 높이를 더한게 발판의 y좌표 +10보다 높을 때
            {
                pPlayer->Jump();    // 개구리가 점프함
                return true;
            }
        }
        return false;
...
int main(void)
{
    ...
    while (window.isOpen())
    {
        ...
        pPlayer->Draw(window);
        pBar->Draw(window);
        if (pPlayer->GetY() >= 700) {   // 개구리가 바닥에 닿았을 때   // 수정 필요
            pPlayer->Stop();   // speed를 0으로
